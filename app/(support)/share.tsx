...
export default function ShareScreen() {
    ...
    const handleShare = () => {
        Share.share({ message: `${t('share.shareMessage')}\n${APP_URL}` });
    };
    ...
    return (
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            <DefaultView style={styles.heroSection}>
                ...
                <Text style={styles.heroTitle}>{t('share.title')}</Text>
                <Text style={styles.heroSubtitle}>{t('share.subtitle')}</Text>
            </DefaultView>
            ...
            <Pressable onPress={handleShare} style={({ pressed }) => [styles.shareBtn, { opacity: pressed ? 0.85 : 1 }]}>
...
const styles = StyleSheet.create({
    ...
    heroIcon: { width: 90, height: 90, borderRadius: 45, alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
    heroTitle: { fontSize: 24, fontWeight: '700', marginBottom: 10 },
    heroSubtitle: { fontSize: 15, color: Colors.palette.textMuted, textAlign: 'center', lineHeight: 22, paddingHorizontal: 20 },
    linkCard: { flexDirection: 'row', alignItems: 'center', width: '100%', padding: 16, borderRadius: 14, marginBottom: 20, gap: 10 },
    linkText: { flex: 1, fontSize: 14, color: Colors.palette.textMuted },
    shareBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', width: '100%', height: 54, borderRadius: 27, backgroundColor: Colors.palette.primary },
    shareBtnText: { fontSize: 17, fontWeight: '700', color: Colors.palette.white },
});
