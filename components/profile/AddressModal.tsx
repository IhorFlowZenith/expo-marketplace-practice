...
function LabeledInput({ label, value, onChangeText, placeholder, inputBg, textColor, borderColor, icon }: LabeledInputProps) {
    return (
...
export default function AddressModal({ visible, initialData = { street: '', city: '', country: '', postalCode: '' }, onSave, onClose }: AddressModalProps) {
    ...
    const handleSave = () => {
        if (!street.trim()) { setError(t('modals.errors.enterStreet')); return; }
        if (!city.trim()) { setError(t('modals.errors.enterCity')); return; }
        if (!country.trim()) { setError(t('modals.errors.enterCountry')); return; }
        if (!postalCode.trim()) { setError(t('modals.errors.enterPostal')); return; }
            ...
            <Pressable style={styles.overlay} onPress={handleClose}>
                <Pressable style={[styles.content, { backgroundColor: cardBg }]} onPress={() => {}}>
                    <DefaultView style={styles.header}>
                        <Text style={styles.title}>{t('modals.deliveryAddress')}</Text>
                        <Pressable onPress={handleClose} style={({ pressed }) => [styles.closeBtn, { opacity: pressed ? 0.6 : 1 }]}>
                            <Ionicons name="close" size={22} color={textColor} />
                        </Pressable>
                    </DefaultView>

                    <LabeledInput label={t('modals.streetAddress')} value={street} onChangeText={(v) => { setStreet(v); setError(null); }} placeholder="123 Main Street" icon="home-outline" inputBg={inputBg} textColor={textColor} borderColor={borderColor} />
                    <LabeledInput label={t('modals.city')} value={city} onChangeText={(v) => { setCity(v); setError(null); }} placeholder="New York" icon="business-outline" inputBg={inputBg} textColor={textColor} borderColor={borderColor} />

                    <DefaultView style={{ flexDirection: 'row', gap: 12 }}>
                        <DefaultView style={{ flex: 1 }}>
                            <LabeledInput label={t('modals.country')} value={country} onChangeText={(v) => { setCountry(v); setError(null); }} placeholder="USA" icon="globe-outline" inputBg={inputBg} textColor={textColor} borderColor={borderColor} />
                        </DefaultView>
                        <DefaultView style={{ flex: 1 }}>
                            <LabeledInput label={t('modals.postalCode')} value={postalCode} onChangeText={(v) => { setPostalCode(v); setError(null); }} placeholder="10001" icon="mail-outline" inputBg={inputBg} textColor={textColor} borderColor={borderColor} />
                        </DefaultView>
                    </DefaultView>
                    ...
                    <DefaultView style={styles.actions}>
                        ...
                        <Pressable onPress={handleSave} style={({ pressed }) => [styles.saveBtn, { opacity: pressed ? 0.85 : 1 }]}>
                            <Text style={styles.saveText}>{t('modals.saveAddress')}</Text>
                        </Pressable>
                    </DefaultView>
                </Pressable>
            </Pressable>
...
const styles = StyleSheet.create({
    ...
    content: { borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingHorizontal: 24, paddingTop: 20, paddingBottom: 36 },
    ...
    inputLabel: { flexDirection: 'row', alignItems: 'center', marginBottom: 6 },
    inputLabelText: { fontSize: 13, fontWeight: '600', color: Colors.palette.textMuted },
    input: { height: 48, borderRadius: 12, paddingHorizontal: 14, fontSize: 15, borderWidth: 1 },
    errorText: { color: Colors.palette.error, fontSize: 13, marginBottom: 12, marginLeft: 2 },
