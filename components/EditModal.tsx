...
export default function EditFieldModal({ visible, field, value, onChangeText, onSave, onClose }: EditFieldModalProps) {
            ...
            <Pressable style={styles.overlay} onPress={onClose}>
                <Pressable style={[styles.content, { backgroundColor: cardBg }]} onPress={() => {}}>
                    <DefaultView style={styles.header}>
                        <Text style={styles.title}>{t('modals.edit')} {meta?.label}</Text>
                    ...
                    <TextInput
                        ...
                        autoFocus
                        placeholder={`${t('modals.enterField')} ${meta?.label?.toLowerCase()}`}
                        placeholderTextColor={Colors.palette.placeholder}
                    />

                    <DefaultView style={styles.actions}>
                        <Pressable onPress={onClose} style={({ pressed }) => [styles.cancelBtn, { borderColor, opacity: pressed ? 0.7 : 1 }]}>
                            <Text style={styles.cancelText}>{t('modals.cancel')}</Text>
                        </Pressable>
                        <Pressable onPress={onSave} style={({ pressed }) => [styles.saveBtn, { opacity: pressed ? 0.85 : 1 }]}>
                            <Text style={styles.saveText}>{t('modals.save')}</Text>
                        </Pressable>
                    </DefaultView>
                </Pressable>
            </Pressable>
...
const styles = StyleSheet.create({
    ...
    input: { height: 52, borderRadius: 14, paddingHorizontal: 16, fontSize: 16, borderWidth: 1, marginBottom: 20 },
